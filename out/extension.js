...
class CottonUtils {
    ...
    static getPathVariations(componentName) {
    ...
    static async findComponentFile(componentName) {
    ...
    static findCottonComponentAtPosition(document, position) {
...
class CottonCompletionProvider {
    async provideCompletionItems(document, position, token) {
        ...
        const enhancedItems = await Promise.all(completionItems.map(async (item) => this.enhanceCompletionItem(item, document, position, partialName)));
        // Filter items based on what's been typed
        if (partialName) {
            const filteredItems = enhancedItems.filter(item => item.label.toString().toLowerCase().startsWith(partialName.toLowerCase()));
            ...
            for (const entry of entries) {
                ...
                if (entry.isDirectory()) {
                    ...
                    const componentName = currentRelativePath
                        ...
                        .replace(/[\\/]/g, '.') // Replace slashes with dots
                        .replace(/_/g, '-'); // Replace underscores with hyphens
                    const completionItem = this.createCompletionItem(componentName, path.join(basePath, currentRelativePath));
                    items.push(completionItem);
                }
            }
        }
    ...
    async enhanceCompletionItem(item, document, position, partialName) {
        try {
            ...
            if (openingTagInfo) {
                ...
                const replaceRange = new vscode.Range(document.positionAt(componentNameStart), document.positionAt(end));
                if (isSelfClosing) {
                    ...
                    if (closingTagInfo) {
                        ...
                        const additionalTextEdits = [
                            vscode.TextEdit.replace(new vscode.Range(document.positionAt(closingTagInfo.start), document.positionAt(closingTagInfo.end)), `</c-${componentName}>`)
                        ];
                        item.insertText = new vscode.SnippetString(`${componentName}\${0}`);
                        item.range = replaceRange;
                        ...
                        if (closingBracketIndex !== -1) {
                            ...
                            const additionalTextEdits = [
                                vscode.TextEdit.insert(document.positionAt(closingBracketPosition), `</c-${componentName}>`)
                            ];
                            item.insertText = new vscode.SnippetString(`${componentName}\${0}`);
                            item.range = replaceRange;
                            item.additionalTextEdits = additionalTextEdits;
    ...
    findCurrentOpeningTag(documentText, currentOffset) {
        ...
        console.log(`Cotton: Tag boundaries: ${result.start}-${result.end}, component: "${result.currentComponentName}", self-closing: ${result.isSelfClosing}`);
        return result;
    }
    findMatchingClosingTag(documentText, currentOffset, currentComponentName) {
...
class CottonAttributeCompletionProvider {
    ...
    parseExistingAttributesMultiLine(document, position, componentName) {
        const documentText = document.getText();
        const offset = document.offsetAt(position);
